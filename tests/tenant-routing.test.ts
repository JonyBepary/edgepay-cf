...
async function createMerchant(name: string): Promise<number> {
  ...
  const row = await testEnv.DB.prepare(`SELECT id FROM op_merchants WHERE uuid = ? LIMIT 1`).bind(uuid).first<{ id: number }>();
  return row!.id;
}
...
describe('authenticated tenant mismatch (API key and JWT)', () => {
  ...
  beforeAll(async () => {
    ...
    const jwtService = createJwtService(testEnv);
    ...
    await testEnv.KV.put('system:bootstrapped', 'true');
    // Clear any cached domain entries for fresh lookup
    await testEnv.KV.delete(`domain:${domainA}`);
    await testEnv.KV.delete(`domain-v2:${domainA}`);
  });
  ...
  it('master domain (APP_DOMAIN) bypasses tenant check for any merchant key', async () => {
    const master = testEnv.APP_DOMAIN ?? 'edgepay-cf.bm-jonybepary.workers.dev';
